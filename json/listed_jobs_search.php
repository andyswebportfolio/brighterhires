...
//Get raw results
$results_length = 
$data = pdo_return("SELECT `page_link` FROM `listed_jobs` WHERE `username` = '".$_SESSION['username']."' AND `user_delete_marked` = 0 ");

//*this actually gets the page link, so do that
...
$array2 = array();
for ($i=0; $i<count($array); $i++) {
 $listing_data = pdo_return_arr("SELECT `company_name`,`job_title`,`job_description`,`company_logo`,`working_hours`,`unix_time`,`page_link`,`listing_live` FROM `listed_jobs` WHERE `page_link` = '".$array[$i]."' ")[0];
 
 //Shorten strings
 $listing_data['job_description'] = mb_substr($listing_data['job_description'],0,221).'...';
 
 //Edit strings
 $listing_data['unix_time'] = 'Posted on '.date('j/n/y',$listing_data['unix_time']);
